...
while the implemented output layers contain 64 or 65 units. The reference and
present image models both use ResNet-50, but the present model is additionally
refined using selective unfreezing, frozen batch-normalization statistics, and
...
\begin{tabular}{p{0.20\columnwidth}p{0.32\columnwidth}p{0.34\columnwidth}}
\hline
\textbf{Aspect} & \textbf{Reference \cite{charan2022drone}} &
\textbf{Present Work} \\
\hline
Modalities & Image; position; position + height; position + height + distance
& Same four modalities \\
Beam classes & 32, downsampled from 64 power values & 64 or 65, depending on
the model \\
Position-family split & 8,402 train; 3,602 test (70/30) & 6,832 train; 3,416
validation; 1,139 test (60/30/10) \\
Position MLP & Two 512-unit hidden layers & Three 512-unit hidden layers \\
Position batch size & 32 & 128 \\
Vision model & ResNet-50 & Refined ResNet-50 \\
Vision batch size & 32 & 64 \\
Missing height/distance & Not reported & 617 of 11,387 samples; median
imputation \\
Metrics & Top-$k$ accuracy & Top-1, Top-2, Top-3, and Top-5 accuracy \\
\hline
\end{tabular}
\end{table}
